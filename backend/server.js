...
app.post("/api/chat", async (req, res) => {
  try {
    const response = await fetch("http://host.docker.internal:5001/chat", {
      method: "POST",
    ...
    res.status(503).json({
      error: "Chat service is not available. Please make sure the Python chat service is running on port 5001."
    });
  }
});
...
app.post("/api/demand-prediction", async (req, res) => {
  try {
    const response = await fetch("http://host.docker.internal:5001/demand-prediction", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(req.body)
    });
    ...
    res.status(503).json({
      error: "Demand prediction service is not available. Please make sure the Python service is running on port 5001."
    });
  }
});
